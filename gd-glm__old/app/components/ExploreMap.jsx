...
class ExploreMap extends Component {
    ...
    displayOverlappingMarkers = (featuresAtPixel: ol.features, theMap: ol.Map) => {
        ...
        this.setState({
            expandedClusterLayer: newFeaturesLayer, multiLineLayer, expandedCluster: true
        });
    };
    ...
    popupHandler = (theMap: ol.Map, e: InputEventMap, dragEvent: boolean = false) => {
        ...
        if (dragEvent === true) {
            ...
            if (typeof e !== 'undefined') {
                const featuresAtPixel = theMap.forEachFeatureAtPixel(e.pixel, function (featureChange) {
    ...
    popupHandleHelper = (feature: ol.Feature, coordinate: number[], overlay: ol.Overlay, theMap: ol.Map) => {
        if (feature && feature.getId()) {
            ...
            const sensorInfo = feature.attributes;
            const lonLatPoint = [sensorInfo.longitude, sensorInfo.latitude];
            const webMercatorPoint = ol.proj.fromLonLat(lonLatPoint);
            ...
            const centerCoordinates = feature.getGeometry().getCoordinates();
            if (theMap.getView().getZoom() <= 7) {
                centerCoordinates[1] += 200000;
            } else {
                centerCoordinates[1] += 25000;
            }
            ...
            const AreaLocationID = matchMapArea(lonLatPoint);

            // Area Information from the Configuration File
            const area = getAreaLocations(AreaLocationID);
    ...
    mapDidUpdate = (theMap: ol.Map) => {
        ...
        const { exploreLayersDetails, layersVisibility } = this.props;
        ...
        if (exploreLayersDetails) {
            ...
            exploreLayersDetails.map(layerDetails => {
                ...
                if (layersVisibility) {
                    ...
                    const index = layersVisibility.findIndex(
        ...
        if (exploreLayers.length > 0) {
            ...
            const all_map_layers = theMap.getLayers().getArray().slice();
            all_map_layers.map(map_layer => {
                const layer_name = map_layer.get('name');
                exploreLayers.map(explore_layer_remove => {
                    const explore_layer_name = explore_layer_remove.get('name');
            ...
            exploreLayers.map(explore_layer_add => {
                const explore_layers_visibility = explore_layer_add.get('visible');
        ...
        const tmpvectorSource = new ol.source.Vector({
            features
        });

        const vectorExtent = tmpvectorSource.getExtent();
        ...
        if (that.props.showPopup === true) {
            const featuresAtPixel = tmpvectorSource.getFeatures().find(function (feature) {
            ...
            const centerCoordinates = featuresAtPixel.getGeometry().getCoordinates();
            centerCoordinates[1] += 500;
            theMap.getView().setCenter(centerCoordinates);
        }
    ...
    mapDidMount = () => {
        const { resetDetailPageSelection } = this.props;
        resetDetailPageSelection();
    };

    componentWillMount() {
        const areaPolygonSource = new ol.source.Vector({
            features: [
                new ol.Feature({})
            ]
        });

        this.setState({ areaPolygonSource });
    }

    getCustomLayers = () => {
        const areaPolygonLayer = new ol.layer.Vector({
            id: 'areaPolygon',
    ...
    getFeature() {
        ...
        if (screen.width <= getMobileSizeMax()) {
            const mobile_sourcenames = getMobileSourceNames().toUpperCase();
        ...
        return new ol.layer.Vector({
            ...
            style (feature) {
                ...
                if (size > 1) {
                    ...
                    const featureColor = feature.getProperties().features[0].attributes.color;
    ...
    render() {
        const { disable_clusters } = this.props;
