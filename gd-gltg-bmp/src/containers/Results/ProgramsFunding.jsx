    ...
    prepareData: (filters: Filters, data: { [k: string]: any }) => {
        ...
        data.forEach((d, idx) => {
            ...
            if (chartDataObject[d.program]) {
                chartDataObject[d.program][boundaryId] = d['funding-sum'];
            } else {
                chartDataObject[d.program] = {
                    [boundaryId]: d['funding-sum']
                };
            }

            if (tableDataObject[boundaryId]) {
                tableDataObject[boundaryId][d.program] = d['funding-sum'];
            } else {
                tableDataObject[boundaryId] = {
                    color,
                    [d.program]: d['funding-sum']
                };
            }
        });
