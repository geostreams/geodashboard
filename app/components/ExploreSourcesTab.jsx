...
class ExploreSourcesTab extends Component {
    ...
    clickSensor = (id, coordinates, e) => {
       this.props.onSelectClick(id, coordinates.slice(0,2));
    }
    ...
    render() {
        ...
        let tabs = this.props.data.filter(data => data.properties.type.id === this.props.source.id)
            .map(data =>
                <FlatButton label={data.name} key={data.id} onClick={this.clickSensor.bind(this, data.id, data.geometry.coordinates)}/>
            );
        return (
            <div>
                {tabs}
